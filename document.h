  ...
  Q_INVOKABLE void load(QUrl fileUrl) {
    ...
    if (root.contains("data") && root["data"].isArray()) {
      ...
      for (int i = 0; i < data.size(); ++i) {
        ...
        if (point.contains("x") && point["x"].isDouble() &&
            point.contains("y") && point["y"].isDouble() &&
            point.contains("nl80211_scan") && point["nl80211_scan"].isArray()) {
          ...
          for (int j = 0; j < nl80211_scan.size(); ++j) {
            ...
            if (scanItem.contains("bssid") && scanItem["bssid"].isString() &&
                ...
                scanItem.contains("channel") &&
                scanItem["channel"].isDouble()) {
